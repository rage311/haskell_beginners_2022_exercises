...
-}
makeSnippet :: (Int -> String -> String) -> String -> String
...
minmax x y z = minme x [y, z]
    where
        minme :: Int -> [Int] -> Int
        minme = current_min rest
            | null rest = current_min
            | (head rest) > current_min = minme (head rest) (tail rest)
            | otherwise = minme current_min (tail rest)
...
-}
strSum :: String -> Int
strSum str = foldl1 (+) (map (\x -> read x :: Int) (words str))
...
lowerAndGreater n list = accum 0 0 list
    where
        accum :: Int -> Int -> [Int] -> [Int]
        accum less greater remaining =
            | null remaining = [less, greater]
            | (head remaining) > n = accum less (greater + 1) (tail remaining)
            | (head remaining) < n = accum (less + 1) greater (tail remaining)
            | otherwise = accum less greater (tail remaining)
